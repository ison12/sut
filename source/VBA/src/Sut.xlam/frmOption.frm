...
' カラム書式設定
Private WithEvents columnTypeFormatter As sutredlib.dbColumnTypeFormatter
Attribute columnTypeFormatter.VB_VarHelpID = -1
...
Private Sub initial()
    ...
    ' カラム書式設定を初期化する
    Set columnTypeFormatter = New sutredlib.dbColumnTypeFormatter
...
Private Sub unInitial()
    ...
    ' カラム書式設定を破棄する
    Set columnTypeFormatter = Nothing
...
Private Sub cmdColumnTypeFormatOracle_Click()
    ...
    settingColFormatDb = DbmsType.Oracle
    
    columnTypeFormatter.SetDbColumnInfo dbColInfo.convertAliasDataType
    columnTypeFormatter.Show
End Sub
...
Private Sub cmdColumnTypeFormatMySQL_Click()
    ...
    settingColFormatDb = DbmsType.MySQL
...
Private Sub cmdColumnTypeFormatPostgreSQL_Click()
    ...
    settingColFormatDb = DbmsType.PostgreSQL
...
Private Sub cmdColumnTypeFormatSQLServer_Click()
    ...
    settingColFormatDb = DbmsType.MicrosoftSqlServer
...
Private Sub cmdColumnTypeFormatAccess_Click()
    ...
    settingColFormatDb = DbmsType.MicrosoftAccess
...
Private Sub cmdColumnTypeFormatSymfoware_Click()
    ...
    settingColFormatDb = DbmsType.Symfoware
...
'
' =========================================================
Private Sub columnTypeFormatter_OnLoad(ByVal info As sutredlib.IDbColumnTypeDbInfo)

    ' アプリケーション設定情報にロードされた情報を設定する
    applicationSettingColFmt.setDbColFormatInfo info
    
    ' レジストリに情報を書き込む
    applicationSettingColFmt.writeForRegistryDbInfo applicationSettingColFmt.getDbColFormatInfo(info.dbName)

End Sub

' =========================================================
' ▽カラム書式設定ウィンドウのデフォルトボタン押下時のイベントプロシージャ
'
' 概要　　　：
' 引数　　　：
' 戻り値　　：
'
' =========================================================
Private Sub columnTypeFormatter_OnDefault()
    ...
    Set dbColumnType = dbObjFactory.createColumnType(CStr(settingColFormatDb))
    
    Set dbColumnFmtInfo = New ValDbColumnFormatInfo
    dbColumnFmtInfo.dbName = CStr(settingColFormatDb)
    Set dbColumnFmtInfo.columnList = dbColumnType.getDefaultColumnFormat
    
    ' デフォルト設定をダイアログに反映する
    columnTypeFormatter.SetDbColumnInfo dbColumnFmtInfo.convertAliasDataType
