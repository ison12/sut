...
'
' =========================================================
Public Function SutShowLicenceInfo()
    ...
    ' ライセンス情報の取得
    Dim lInfo As Object
    Set lInfo = getLicenceInfo
    
    menuHelp.init isAuthenticatedLicence, lInfo
    menuHelp.showLicenceInfo
...
' =========================================================
' ▽ライセンス情報が認証されているかを確認する。
'
' 概要　　　：
'
' =========================================================
Public Function isAuthenticatedLicence() As Boolean

    Dim author As New ExeAuthenticateLicence
    author.init getLicenceInfo
    
    ' 認証成功
    If author.executeAuthor = True Then
    
        isAuthenticatedLicence = True
    
    ' 認証失敗
    Else
        isAuthenticatedLicence = False
...
'
' =========================================================
Public Function getLicenceInfo() As Object

    ' 初期化されている場合
    If Not licenceInfo Is Nothing Then
    
    
    ' 初期化されていない場合
    Else
    
        Set licenceInfo = New ValLicenceInfo
        licenceInfo.readForRegistry
        
    End If

    ' 戻り値を設定する
    Set getLicenceInfo = licenceInfo
...
' ▽試用期間日の確認
'
' 概要　　　：試用期間を過ぎている場合にライセンス登録フォームを開く
'
' =========================================================
Public Function checkProbationDay()

    ' 未認証で且つ、試用期間が過ぎている場合にライセンス登録用のダイアログを表示する
    #If CHECK_PROB_DAY <> 1 Then
    
        ' 未認証の場合
        If isAuthenticatedLicence = False Then
            ...
            ' 試用期間の範囲内であるかを確認
            If author.isRangeProbation(fromDate) = False Then
            
                ' 範囲外なので、ライセンス登録フォームを開く
                menuHelp.init False, getLicenceInfo
                menuHelp.showLicenceInfo
            End If
...
Private Function initToolbar()
    ...
    ' ライセンスをコマンドバーにボタンを追加する
    Set btnLicence = popHelp.Controls.Add(Type:=msoControlButton)
    ...
    With btnLicence
        ...
        .Style = msoButtonIconAndCaption
        .Caption = "ライセンスについて"
        .DescriptionText = "ライセンスについて"
        .OnAction = "Main.SutShowLicenceInfo"
        .Tag = COMMANDBAR_CONTROL_BASE_ID & "Main.SutShowLicenceInfo"
        ' Excel2002以降のプロパティ
        If excelVer >= Ver2002 Then
            setCommandBarControlIcon btnLicence _
                                   , IDB_ICON_SMILE _
                                   , IDB_ICON_SMILE_MASK
        End If
